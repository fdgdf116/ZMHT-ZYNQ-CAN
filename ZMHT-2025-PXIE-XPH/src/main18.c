...
static uint64_t g_bcast_rx_gate_until_ms = 0;
...
static void reset_broadcast_buffers(void) {
    ...
    g_bcast_rx_gate_open = 0;
    g_bcast_rx_gate_until_ms = 0;
...
static void set_broadcast_rx_gate(uint8_t open) {
    ...
    g_bcast_rx_gate_until_ms = open ? (monotonic_ms() + BCAST_PPS_RX_WINDOW_MS) : 0;
    pthread_mutex_unlock(&bcast_rx_gate_mutex);
}
...
static int is_broadcast_rx_gate_open(void) {
    ...
    pthread_mutex_lock(&bcast_rx_gate_mutex);
    open = (g_bcast_rx_gate_open && monotonic_ms() <= g_bcast_rx_gate_until_ms);
    if (!open) {
        g_bcast_rx_gate_open = 0;
        g_bcast_rx_gate_until_ms = 0;
    }
...
static int is_broadcast_pps_window_open(uint8_t ch) {
    ...
    return is_broadcast_rx_gate_open();
}
